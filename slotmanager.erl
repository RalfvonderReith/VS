...
communicate(SlotList, Size, SNo) ->
	...
	receive
		...
		{reserve, Slot} ->
			...
			communicate(reserveSlot(SlotList, Slot), Size, SNo);
		{get_slot, PID} -> 
...
reserveSlotH([], CheckedSlots, Slot, SNo) -> 
	...
	CheckedSlots;
reserveSlotH([Slot|Rest], CheckedSlots, Slot, SNo) -> lists:concat([CheckedSlots, Rest]);
reserveSlotH([First|Rest], CheckedSlots, Slot, SNo) -> reserveSlotH(Rest, lists:append(CheckedSlots, [First]), Slot).
